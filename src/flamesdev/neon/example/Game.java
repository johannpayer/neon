...
public class Game implements IGame {
    ...
    private Rectangle obstacle = new Rectangle(new RectangularHitbox(new Vector2D(0.5, 0.7), 0.2, 0.2), Color.green, true);
    private Rectangle obstacle2 = new Rectangle(new RectangularHitbox(new Vector2D(0.5, 0.3), 0.15, 0.15), Color.orange, true);

    @Override
    public void init() {
        rect = new Rectangle(new RectangularHitbox(new Vector2D(0.5, 0.5), 0.1, Units.toHeight(0.1)), Color.red, true);
    }

    @Override
    public void tick() {
        rect.setColor(InputSystem.leftMouseButtonHeld() ? Color.red : Color.blue);
        RectangularHitbox hitbox = rect.getHitbox();
    ...
    public void render(Graphics graphics) {
        RenderSystem.drawImage(graphics, new ImageObject(new RectangularHitbox(new Vector2D(0.15, 0.85), 0.3, 0.3), buff));
